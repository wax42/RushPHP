...
<?PHP foreach ($catall as $c)
            {
                ...
                { ?>
<div class="cat">
    <?PHP $items = db_select_item_cat($c['name']);?>
    <a href=index.php?cat=<?PHP echo $c['name'];?>><h1 class="cat_acceuil"> <?PHP echo $c['name'];?> </h1></a>
    <?PHP 
        $count = 0;
        foreach ($items as $it)
        {?>
    ...
    <img class="article_img" src="images/<?PHP echo $it['img']?>"> </img>
        ...
        <form method="post" action="index.php?cat=<?PHP echo $c['name'];?>">
